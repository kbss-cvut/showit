...
const getSearchResults = async (word: string | undefined) => {
  ...
  const result = Object.entries(grouped).map(([key, objs]) => {
    return {
      type:
        objs.length === 1
      ...
      displayText: objs[0].snippetField === "label" ? objs[0].snippetText : key,
      total_score: maxBy(objs, "score")?.score,
      items: [...objs],
      isMatchInDefinition: objs[0].snippetField === "definition",
      //When there is multiple matches, we cannot decide which snippet to show
      snippetText:
        objs.length !== 1 ? "<em>Nalezeno více shod</em>" : objs[0].snippetText,
  ...
  const result2 = vocabularyData.map((item) => {
    return {
  ...
  result.push(...result2);
  const finalResult = orderBy(result, "total_score", "desc");
